...
const ChangeViewButton = ({ text, iconRotation, onClick, className = "" }: ChangeViewButtonProps) => {
  return (
    <button
      onClick={onClick}
    ...
    >
      <span>{text}</span>
      <span className={`transform transition-transform duration-300 ${iconRotation}`}>
        ↓
      </span>
    </button>
  );
